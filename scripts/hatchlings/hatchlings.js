...
var ACCOUNT_COMBAT_FLAGS = (/* unused pure expression or super */ null && (["aabosses", "wowbar", "bothcombatinterf", "compactmanuel", "eternalmrj", "disablelovebugs"]));
/**
...
function setCombatFlags() {
  ...
  return visitUrl("account.php?".concat(([].concat(lib_toConsumableArray(flat(flags.map(_ref9 => {
    var flag = _ref9.flag,
        value = _ref9.value;
...
function withCombatFlags(action) {
  ...
  var initialValues = getCombatFlags(flags.map(_ref10 => {
...
var Outfit = /*#__PURE__*/function () {
  ...
  outfit_createClass(Outfit, [{
    ...
    value: function spec() {
      ...
      var riders = {};
      var buddyRider = this.riders.get($slot(_templateObject52 || (_templateObject52 = outfit_taggedTemplateLiteral(["buddy-bjorn"]))));
...
;// CONCATENATED MODULE: ./src/lib/args.ts

var args = Args.create("hatchlings", "A simple script to help you find and use familiar hatchlings you didn't know you had.", {
  use: Args.flag({
    setting: "",
    default: false,
...
function main(command) {
  ...
  var hatchlings = $familiars(main_templateObject || (main_templateObject = main_taggedTemplateLiteral([""]))).filter(fam => !have(fam));
  ...
  var haveFams = hatchlings.filter(fam => have(fam.hatchling));
  ...
  if (!args.buy) {
    (0,external_kolmafia_namespaceObject.print)("Found ".concat(haveFams.length, " hatchlings in your inventory that you haven't hatched before..."), "green");
    var output = ["<table border=2 cols=3><tr><th>Familiar</th><th>Hatchling</th><th>Mall Price</th></tr>"];
    haveFams.forEach(fam => {
      output.push("<tr>" + "<td><p>".concat(fam, "</p></td>") + "<td><p>".concat(fam.hatchling, "</p></td>") + "<td><p>".concat((0,external_kolmafia_namespaceObject.toString)((0,external_kolmafia_namespaceObject.retrievePrice)(fam.hatchling), "%,d"), "</p></td>") + "</tr>");
    });
    output.push("</table>");
...
function buyItems(fams) {
  var dontHave = fams.filter(fam => !have(fam.hatchling));
  var totalPrice = 0;
  var hatchlingCount = 0;
  dontHave.forEach(fam => {
    var itemPrice = (0,external_kolmafia_namespaceObject.retrievePrice)(fam.hatchling);
    ...
    if (itemPrice > 0 && itemPrice < args.buyLimit) {
      ...
      if (args.sim) {
        (0,external_kolmafia_namespaceObject.print)("Sim: Buying 1 ".concat(fam.hatchling, " at ").concat((0,external_kolmafia_namespaceObject.toString)(itemPrice, "%,d")));
      } else {
        (0,external_kolmafia_namespaceObject.buy)(1, fam.hatchling, args.buyLimit);
      }
    }
  });

  if (args.sim) {
    (0,external_kolmafia_namespaceObject.print)("You'd spend ".concat((0,external_kolmafia_namespaceObject.toString)(totalPrice, "%,.0f"), " meat to get ").concat(hatchlingCount, " new hatchlings"), "green");
  }
}

function useHatchlings(inv) {
  (0,external_kolmafia_namespaceObject.print)("Found ".concat(inv.length, " familiar hatchlings in your inventory that you haven't hatched before..."), "green");
  inv.forEach(item => {
    if (args.sim) {
      simUsage(item);
    } else {
...
function simUsage(fam) {
  var price = (0,external_kolmafia_namespaceObject.retrievePrice)(fam.hatchling);

  if (args.useLimit > 0 && price > args.useLimit) {
    (0,external_kolmafia_namespaceObject.print)("Sim: Skipping ".concat(fam.hatchling, " because it's too expensive. (").concat((0,external_kolmafia_namespaceObject.toString)(price, "%,d"), " meat)"), "orange");
  } else {
    (0,external_kolmafia_namespaceObject.print)("Sim: Using 1 ".concat(fam.hatchling));
  }
}

function realUsage(fam) {
  var price = (0,external_kolmafia_namespaceObject.retrievePrice)(fam.hatchling);

  if (args.useLimit > 0 && price > args.useLimit) {
    (0,external_kolmafia_namespaceObject.print)("Skipping ".concat(fam.hatchling, " because it's too expensive. (").concat((0,external_kolmafia_namespaceObject.toString)(price, "%,d"), " meat)"), "orange");
  } else {
    (0,external_kolmafia_namespaceObject.use)(1, fam.hatchling);
  }
}
